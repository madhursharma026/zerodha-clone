    ...
    io.on('connection', (socket) => {
      ...
      const sendPrices = async () => {
        ...
        for (const symbol of Object.keys(priceFeeds)) {
          ...
          let price = Number(data.answer) / 1e8

          // 🔧 Simulate small price changes for testing (±0.5%)
          const randomFluctuation = 1 + (Math.random() - 0.5) * 0.01
          price = price * randomFluctuation

          prices[symbol] = Number(price.toFixed(8))
        }
